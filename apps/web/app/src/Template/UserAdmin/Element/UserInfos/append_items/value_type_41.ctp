...
?>
        ...
        <?= $this->Form->input("info_append_items.{$num}.value_decimal", ['type' => 'checkbox', 'value' => '1', 'hiddenField' => true, 'label' => '別タブ表示']); ?>

        <?= empty($length) ? '' : '<br><span>※' . h($length) . '文字以内で入力してください</span>';?>
        <?= empty($notes) ? '' : '<br><span>※' . h($notes) . '</span>';?>
        <br><span>※デフォルトは同タブ表示です</span>
        ...
        <?= h($data['value_text'] ?? '') ?>
        <br>
        <?= $data['value_decimal'] ? ($data['value_decimal'] == 1 ? '別タブ表示' : '同タブ表示') : '' ?>
