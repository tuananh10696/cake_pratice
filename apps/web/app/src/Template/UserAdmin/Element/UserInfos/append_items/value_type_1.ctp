...
?>
        <?= $this->Form->input("info_append_items.{$num}.value_int", ['type' => 'text', 'maxlength' => $length, 'placeholder' => h($append['value_placeholder'] ?? ''), 'default' => empty($append['value_default']) ? '' : h($append['value_default'])]); ?>
