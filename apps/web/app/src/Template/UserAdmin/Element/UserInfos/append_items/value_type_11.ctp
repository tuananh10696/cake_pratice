...
?>
        ...
        <?= $this->Form->input("info_append_items.{$num}.checkbox_array", ['type' => 'select', 'multiple' => 'checkbox', 'options' => $list, ]);?>

        <?php if (false): ?>
        <?php foreach ($list as $k => $v): ?>
        <?=$this->Form->input('info_append_items.' . $num . '.value_decimal.' . $k, array('type' => 'checkbox', 'value' => $k, 'id' => "append_{$k}_check", 'label' => false, 'hidden' => 'hidden', 'hiddenField' => false));?>
        <label for="<?= "append_{$k}_check" ?>">
            <span><?= $v ?></span>
        </label>
        <?php endforeach; ?>
        <?php endif; ?>
