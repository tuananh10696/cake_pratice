...
?>
        <?= $this->Form->input("info_append_items.{$num}.value_textarea", ['type' => 'textarea', 'class' => 'old_editor', 'maxlength' => $length, 'style' => 'height:215px;', 'placeholder' => h($append['value_placeholder'] ?? ''), 'default' => empty($append['value_default']) ? '' : h($append['value_default'])]); ?>
