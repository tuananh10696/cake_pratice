<tr id="append_block-<?= $append['slug'] ?>">
    ...
    ?>
        ...
        <?= $this->Form->select("info_append_items.{$num}.value_int", $list, ['style' => 'max-width:200px;', 'default' => $value_default, 'placeholder' => h($append['value_placeholder'] ?? '')]) ?>
        <?php else:?>
        <?= $this->Form->select("info_append_items.{$num}.value_int", $list, ['empty' => '--', 'style' => 'max-width:200px;', 'default' => $value_default, 'placeholder' => h($append['value_placeholder'] ?? '')]) ?>
        <?php endif;?>
